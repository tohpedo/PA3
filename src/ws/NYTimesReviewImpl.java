...
@WebService(endpointInterface = "ws.NYTimesReview")
public class NYTimesReviewImpl implements NYTimesReview {
	...
	public Review[] populate() throws ParseException{
		...
		Review r = new Review("NYTimes", "The Titanic", "Love it!", "Drama", dateTemp, "I really loved this movie" );
		Review x = new Review("NYTimes", "The Departed", "Terrible", "Drama", dateTemp, "Leonardo DiCaprio does a fabulous job in this movie" );
		output[0] = r;
		output[1] = x;
		return output;
	...
	public Review[] parseInfo() throws FileNotFoundException, ParseException{
		...
		while (s.hasNextLine())
		{
			...
			if (temp.length > 2)
			{
				allReviews[reviewCounter].setReviewSource("New York Times");
